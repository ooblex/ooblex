...
class FaceDataset(Dataset):
    ...
    def __init__(self, root_dir: str, person: str, transform=None):
        ...
        self.images = list(self.root_dir.glob("*.jpg")) + list(self.root_dir.glob("*.png"))
...
class FaceSwapTrainer:
    ...
    def train_epoch(self, dataloader_A: DataLoader, dataloader_B: DataLoader, epoch: int):
        """Train one epoch"""
        self.model.train()
        total_loss = 0
        
        pbar = tqdm(zip(dataloader_A, dataloader_B), total=min(len(dataloader_A), len(dataloader_B)))
        for i, (batch_A, batch_B) in enumerate(pbar):
            ...
            # Log metrics
            if i % 100 == 0:
                self.writer.add_scalar('Loss/A', loss_A.item(), epoch * len(dataloader_A) + i)
                self.writer.add_scalar('Loss/B', loss_B.item(), epoch * len(dataloader_B) + i)
...
def main():
    ...
    parser.add_argument('--data-dir', type=str, required=True, help='Path to face dataset')
    ...
    # Dataloaders
    dataloader_A = DataLoader(dataset_A, batch_size=args.batch_size, shuffle=True, num_workers=4)
    dataloader_B = DataLoader(dataset_B, batch_size=args.batch_size, shuffle=True, num_workers=4)
