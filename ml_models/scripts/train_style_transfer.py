...
class StyleDataset(Dataset):
    ...
    def __init__(self, content_dir, transform=None):
        ...
        self.images = list(self.content_dir.glob("*.jpg")) + list(self.content_dir.glob("*.png"))
...
def train_style_transfer(args):
    ...
    style_gram = {layer: gram_matrix(features) for layer, features in style_features.items()}
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.epochs}"):
            ...
            content_loss = nn.functional.mse_loss(output_features['relu2_2'], content_features['relu2_2'])
            
            # Style loss
            style_loss = 0
            for layer in style_gram:
                output_gram = gram_matrix(output_features[layer])
                style_loss += nn.functional.mse_loss(output_gram, style_gram[layer].expand_as(output_gram))
