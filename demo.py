...
class OoblexDemo:
    ...
    def create_test_frame(self, width=640, height=480):
        ...
        # Add text
        cv2.putText(
            frame, "OOBLEX DEMO", (180, 250), cv2.FONT_HERSHEY_BOLD, 1.5, (0, 0, 0), 3
        )
    ...
    def test_redis_pipeline(self):
        ...
        decoded_frame = cv2.imdecode(
            np.frombuffer(retrieved, dtype=np.uint8), cv2.IMREAD_COLOR
        )
...
def main():
    ...
    parser.add_argument(
        "--effect", type=str, help="Test specific effect (FaceOn, CartoonOn, etc.)"
    )
    parser.add_argument("--redis", default="redis://localhost:6379", help="Redis URL")
    parser.add_argument(
        "--rabbitmq", default="amqp://guest:guest@localhost:5672", help="RabbitMQ URL"
    )
