...
def create_test_image():
    ...
    cv2.putText(img, "Test Image", (250, 400), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    for effect in effects:
        try:
            ...
            if effect == 'grayscale' and len(result.shape) == 3:
                ...
                is_grayscale = np.allclose(result[:,:,0], result[:,:,1]) and np.allclose(result[:,:,1], result[:,:,2])
                status = "✓" if is_grayscale else "✗"
                extra = " (converted to grayscale)" if is_grayscale else " (FAILED to convert)"
