...
from aiortc.contrib.media import MediaBlackhole, MediaRelay, MediaRecorder
...
class WHEPResource:
    ...
    def __init__(self, resource_id: str, peer_connection: RTCPeerConnection, source_id: str):
        self.id = resource_id
        self.pc = peer_connection
...
class WHIPWHEPServer:
    ...
    async def handle_whip_post(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            async def on_track(track: MediaStreamTrack):
                logger.info(f"WHIP: Track received: {track.kind} for resource {resource_id}")
    ...
    async def handle_whep_post(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            source_id = request.rel_url.query.get('stream') or request.headers.get('X-Stream-ID')
            if not source_id:
            ...
            links.append(f'</whep/{resource_id}/events>; rel="urn:ietf:params:whep:ext:core:server-sent-events"')
            
            # Layer selection extension
            links.append(f'</whep/{resource_id}/layer>; rel="urn:ietf:params:whep:ext:core:layer"')
        ...
        try:
            ...
            if content_type == 'application/trickle-ice-sdpfrag':
                ...
                if 'layer' in data:
                    ...
                    logger.info(f"WHEP: Layer selection for {resource_id}: {data['layer']}")
    ...
    async def handle_whep_delete(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if source_resource:
                ...
                await self.redis_client.hincrby(
                    f"whip:stream:{resource.source_id}:stats",
                    "viewers",
                    -1
                )
                ...
                for key in keys:
                    if ':stats' not in key:
                        ...
                        if stream_data:
                            ...
                            viewers = await self.redis_client.hget(stats_key, "viewers") or 0
                            
                            stream_info['viewers'] = int(viewers)
                            stream_info['whep_url'] = f"/whep?stream={stream_info['id']}"
        ...
        while True:
            try:
                ...
                for resource_id, resource in self.whip_resources.items():
                    if now - resource.created_at > max_age and len(resource.subscribers) == 0:
