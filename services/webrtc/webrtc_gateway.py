...
websocket_connections = Counter(
    "webrtc_websocket_connections_total", "Total WebSocket connections"
)
active_connections = Gauge("webrtc_active_connections", "Active WebSocket connections")
peer_connections = Counter(
    "webrtc_peer_connections_total", "Total peer connections", ["status"]
)
...
class PeerConnection:
    ...
    async def set_remote_description(self, sdp: Dict[str, str]):
        """Set remote SDP"""
        description = RTCSessionDescription(sdp=sdp["sdp"], type=sdp["type"])
...
class WebRTCGateway:
    ...
    async def handle_websocket(self, websocket, path):
        ...
        try:
            ...
            await websocket.send(json.dumps({"type": "welcome", "peer_id": peer_id}))
    ...
    async def handle_offer(self, peer_id: str, message: Dict[str, Any]):
        ...
        # Send answer back
        await self.send_to_peer(peer_id, {"type": "answer", "sdp": answer})
    ...
    async def handle_get_offer(self, peer_id: str, message: Dict[str, Any]):
        ...
        # Send offer
        await self.send_to_peer(peer_id, {"type": "offer", "sdp": offer})
    ...
    async def handle_transform(self, peer_id: str, message: Dict[str, Any]):
        ...
        await self.redis_client.setex(f"transform:{peer_id}", 3600, transform_type)
        ...
        await self.send_to_peer(
            peer_id, {"type": "transform-ack", "transform": transform_type}
        )
    ...
    async def run(self):
        ...
        ssl_context.load_cert_chain(
            certfile=settings.SSL_CERT_PATH, keyfile=settings.SSL_KEY_PATH
        )
        ...
        async with websockets.serve(
            self.handle_websocket, "0.0.0.0", settings.WEBRTC_PORT, ssl=ssl_context
        ):
