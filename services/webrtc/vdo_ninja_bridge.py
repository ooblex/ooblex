...
class VDONinjaBridge:
    ...
    async def handle_message(
        self, participant: VDONinjaParticipant, message: Dict[str, Any]
    ):
    ...
    async def handle_offer(
        ...
        async def on_track(track: MediaStreamTrack):
            ...
            logger.info(
                f"VDO.Ninja track received: {track.kind} from {participant.stream_id}"
            )
    ...
    async def join_room(
        self,
        participant: VDONinjaParticipant,
        room_id: str,
        ...
        if room.password and room.password != password:
            await participant.send_message(
                {"type": "error", "message": "Invalid room password"}
            )
            return
        ...
        await participant.send_message({"type": "roomState", **room.get_room_state()})
    ...
    async def handle_leave_room(
        ...
        if participant.room_id and participant.room_id in self.rooms:
            ...
            for other in room.participants.values():
                await other.send_message(
                    {"type": "participantLeft", "streamID": participant.stream_id}
                )
    ...
    async def handle_settings_update(
        ...
        if participant.room_id and participant.room_id in self.rooms:
            ...
            if participant.is_director:
                ...
                for other in room.participants.values():
                    await other.send_message(
                        {"type": "settings", "settings": room.settings}
                    )
...
class VDONinjaAPI:
    ...
    def setup_routes(self):
        ...
        self.app.router.add_post(
            "/api/vdo/rooms/{room_id}/composite", self.create_composite
        )
    ...
    async def delete_room(self, request: web.Request) -> web.Response:
        ...
        for participant in list(room.participants.values()):
            await participant.send_message(
                {"type": "roomClosed", "reason": "Room deleted by administrator"}
            )
            await participant.close()
    ...
    async def create_composite(self, request: web.Request) -> web.Response:
        ...
        if not composite_id:
            return web.json_response(
                {"error": "Failed to create composite stream"}, status=500
            )
