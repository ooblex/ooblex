...
class VideoDecoder:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Declare queues
            self.decode_queue = await self.channel.declare_queue('video_decode', durable=True)
            self.frame_exchange = await self.channel.declare_exchange('frames', 'topic', durable=True)
    ...
    async def extract_frames(self, stream_id: str, cap: cv2.VideoCapture, config: Dict[str, Any]):
        ...
        try:
            while stream_id in self.active_streams:
                ...
                await self.redis.set(f"latest_frame_meta:{stream_id}", json.dumps(frame_data))
    ...
    async def publish_frame_event(self, stream_id: str, frame_number: int, metadata: Dict[str, Any]):
        ...
        try:
            ...
            await self.frame_exchange.publish(
                message,
                routing_key=f"frame.{stream_id}"
            )
