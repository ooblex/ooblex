...
class OpenCVProcessor:
    ...
    async def blur_background(self, image: np.ndarray) -> np.ndarray:
        ...
        result = (image * mask_3channel + blurred * (1 - mask_3channel)).astype(
            np.uint8
        )
            ...
            try:
                ...
                if process_type == "face_detection":
                    ...
                    result = await self.processor.apply_blur(
                        image, kernel_size=parameters.get("kernel_size", 15)
                    )
                    ...
                    result = await self.processor.pixelate(
                        image, pixel_size=parameters.get("pixel_size", 10)
                    )
                    ...
                    logger.warning(
                        f"Unknown process type: {process_type}, returning original image"
                    )
                    result = image
