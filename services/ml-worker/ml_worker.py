...
model_load_time = Histogram('ml_model_load_seconds', 'Model loading time', ['model_name'])
...
class MLProcessor:
    ...
    async def process_face_swap(self, image: np.ndarray, model_name: str = "face_swap") -> np.ndarray:
    ...
    async def process_style_transfer(self, image: np.ndarray, model_name: str = "style_transfer") -> np.ndarray:
    ...
    async def process_object_detection(self, image: np.ndarray, model_name: str = "yolov8") -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        try:
            ...
            if results.detections:
                for detection in results.detections:
                    ...
                    # Draw bounding box
                    cv2.rectangle(annotated_image, (x, y), (x + box_w, y + box_h), (0, 255, 0), 2)
            ...
            try:
                ...
                if process_type == "face_swap":
                    result = await self.processor.process_face_swap(
                        image, 
                        model_name=parameters.get("model", "face_swap")
                    )
                elif process_type == "style_transfer":
                    result = await self.processor.process_style_transfer(
                        image,
                        model_name=parameters.get("style", "style_transfer")
                    )
