...
class MJPEGStream:
    ...
    async def add_client(self, response: web.StreamResponse):
        ...
        logger.info(f"Client added to stream {self.stream_id}. Total clients: {len(self.clients)}")
        
    async def remove_client(self, response: web.StreamResponse):
        """Remove a client from this stream"""
        self.clients.discard(response)
        logger.info(f"Client removed from stream {self.stream_id}. Total clients: {len(self.clients)}")
                ...
                try:
                    ...
                    if stream.clients:
                        ...
                        if not frame_data.startswith(b'\xff\xd8'):
                            ...
                            frame_array = frame_array.reshape((480, 640, 3))  # Adjust dimensions
                            _, jpeg_data = cv2.imencode('.jpg', frame_array, 
