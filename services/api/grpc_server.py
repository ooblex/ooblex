...
from main import authenticate_user, create_access_token, get_password_hash, get_user
...
class APIServicer(ooblex_pb2_grpc.APIServiceServicer):
    ...
    async def CreateTask(self, request, context):
        ...
        # Store in Redis
        await self.redis_client.setex(f"task:{task_id}", 3600, task.SerializeToString())

        # Send to RabbitMQ
        await self.rabbitmq_channel.default_exchange.publish(
            Message(body=task.SerializeToString()), routing_key="tasks"
        )
    ...
    async def ListTasks(self, request, context):
        ...
        while True:
            ...
            for key in keys:
                ...
                if task_data:
                    ...
                    if (
                        request.status == ooblex_pb2.TASK_STATUS_UNKNOWN
                        or task.status == request.status
                    ):
                        tasks.append(task)
        ...
        response = ooblex_pb2.ListTasksResponse(
            tasks=page_tasks, total_count=len(tasks)
        )
    ...
    ) -> AsyncIterator[ooblex_pb2.TaskStatusUpdate]:
...
class AuthInterceptor(grpc.aio.ServerInterceptor):
    ...
    async def intercept_service(self, continuation, handler_call_details):
        ...
        try:
            ...
            return grpc.unary_unary_rpc_method_handler(
                lambda request, context: context.abort(
                    grpc.StatusCode.UNAUTHENTICATED, error_message
                )
            )
