...
sudo apt-get remove libsrtp0-dev -y
sudo apt-get remove libnice* -y

sudo apt-get install gtk-doc-tools -y

sudo rm /tmp/src -r && mkdir /tmp/src
cd /tmp/src && sudo rm libnice -r && \
...
sudo make && sudo make install && sudo ldconfig
        ...
        cd libsrtp-2.2.0 && \
        ./configure --prefix=/usr/lib64 --enable-openssl && \
