...
Skips tests if ML dependencies (torch, tensorflow, etc.) are not available.
"""
import pytest
import json
...
# Try to import ML worker - will fail if torch/tensorflow/etc not installed
...
try:
    ...
    # This will fail if torch/tensorflow etc aren't installed
    from ml_worker import ModelManager, MLProcessor, MLWorker
    ML_WORKER_AVAILABLE = True
except ImportError as e:
    # ML dependencies not available - tests will be skipped
    pass

pytestmark = pytest.mark.skipif(
    not ML_WORKER_AVAILABLE,
    reason="ML dependencies (torch, tensorflow, etc.) not available"
)
...
class TestModelManager:
    ...
    async def test_load_model_onnx(self, mocker):
        ...
        # Mock onnxruntime
        mock_ort = mocker.patch("ml_worker.ort")
    ...
    async def test_load_model_torch(self, mocker):
        ...
        # Mock torch
        mock_torch = mocker.patch("ml_worker.torch")
...
class TestMLWorker:
    ...
    async def test_setup(self, ml_worker, mocker):
        """Test worker setup"""
        # Mock connections
        mock_redis = mocker.patch("ml_worker.redis.from_url")
        mock_rabbitmq = mocker.patch("ml_worker.connect_robust")
    ...
    async def test_update_gpu_metrics(self, ml_worker, mocker):
        ...
        # Mock metrics
        mock_gauge = mocker.patch("ml_worker.gpu_memory_usage")
