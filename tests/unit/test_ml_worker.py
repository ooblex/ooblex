...
import numpy as np
import pytest
from ml_worker.ml_worker import MLProcessor, MLWorker, ModelManager
...
class TestMLProcessor:
    ...
    @pytest.mark.asyncio
    async def test_process_face_swap_with_face(
        self, ml_processor, sample_image, mocker
    ):
    ...
    async def test_process_object_detection(self, ml_processor, sample_image):
        ...
        annotated, detections = await ml_processor.process_object_detection(
            sample_image
        )
...
class TestMLWorker:
    ...
    async def test_process_task_success(self, ml_worker, mocker):
        ...
        # Mock Redis get
        image_data = cv2.imencode(".jpg", np.zeros((100, 100, 3), dtype=np.uint8))[
            1
        ].tobytes()
