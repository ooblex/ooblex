    ...
    if "janus" in data:
        if "success" in data["janus"]:
            ...
            if act == "offer":
                ...
                answerSDP(uid, data)
                states[uid] = (
                    "ready"  ## I suppose this should be already set to ready already, but this is the "real" ready
                )
                pushICE(uid, None)
            elif act == "join":
                print("joined room")
                try:
                    ...
                    return
                states[uid] = (
                    "ready"  ## we can start pushing ICE the moment we join the room -- albeit this perhaps seems aggressive.
                )
                configureStream(uid)
...
def answerSDP(uid, data):
    ...
    try:
        ...
        codec[uid] = data["plugindata"]["data"][
            "video_codec"
        ]  ## update reality, despite what might of been provided by client
        msg["jsep"] = data["jsep"]
        msg["request"] = "publish"
    except:
...
def joinRoom(
    uid,
):  ### I SHOULD CONVERT THIS TO JOINANDCONFIGURE. Probably shouldn't create the room until I get the SDP and settings too.
...
def rtspServerMain(rtspSocketMain):
    ...
    while True:
        ...
        clientInfo = {}
        clientInfo["rtspSocket"] = (
            rtspSocketMain.accept()
...
class ServerWorker:
    ...
    def processRtspRequest(
        self, data
    ):  ## RTSP services don't need to redirect -- most importantly, we don't know where to redirect to since no IP is provided
        ...
        try:
            ...
            try:
                ...
                raise Exception("Not JSON from request data")
                return False
        ...
        if requestType == "SETUP":
            if self.state == self.INIT:  ## VIDEO
                ...
                try:
                    self.clientInfo["rtpPort"] = data.split("client_port=")[1].split(
                        "-"
                    )[0]
                except:
                ...
                try:  ##
                    self.clientInfo["stream_token"] = filename.split("/")[3]
                    print(
                        "STREAM TOKEN:", self.clientInfo["stream_token"]
                    )  ## ALSO IS THE UID
                except:  ##
                    print("*******!!!!*****")
                ...
                try:
                    self.clientInfo["rtpPort_audio"] = data.split("client_port=")[
                        1
                    ].split("-")[0]
                except:
                ...
                reply = (
                    ...
                    + str(self.clientInfo["session"])
                    + "\r\nTransport: RTP/AVP/UDP;unicast;client_port="
                    + str(self.clientInfo["rtpPort_audio"])
                    + ";\r\n\r\n"
                )
                print(reply)
                connSocket = self.clientInfo["rtspSocket"][0]
                ...
                print(
                    "filename or Token requested is "
                    + filename
                    + " ... "
                    + filename.split("/")[3]
                )
...
c=IN IP4 "
                    + str(self.clientInfo["rtspSocket"][1][0])
                    + "\r\n\
t=0 0\r\n\
