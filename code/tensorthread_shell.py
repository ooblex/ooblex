...
def tensorThread(something):
    ...
    def sendMessage(
        msg, token
    ):  ## THIS IS USED FOR COMMUNICATION ; needed to provide a response
    ...
    def faceDetection(
        img, token, rid, dectype="trump"
    ):  ## RESULT IS IMAGE BASED TRANSFORMATION
        global graph, r
...
mainChannel_in.queue.declare(
    "tf-controller", arguments={"x-message-ttl": 3600000}
)  ### Spin up more threads if needed; this should be un-used I'd suspect
