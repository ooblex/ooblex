...
class NodeLookup(object):
    def __init__(self, label_lookup_path=None, uid_lookup_path=None):
        if not label_lookup_path:
            label_lookup_path = (
                "./inception/imagenet_2012_challenge_label_map_proto.pbtxt"
            )
...
def process_stream(streamKey):
    ...
    while True:
        try:
            connection = UriConnection(
                "amqps://admin:QYGTGMPLVLFYOPRH@portal-ssl494-22.bmix-dal-yp-42bf8654-c98e-426b-b8e4-a9d19926bfde.steve-seguin-email.composedb.com:39186/bmix-dal-yp-42bf8654-c98e-426b-b8e4-a9d19926bfde"
            )
            break
        except:
            continue
    ...
    CLI = (
        ...
        + str(streamKey)
        + " name=r ! application/x-rtp,media=video ! rtph264depay ! video/x-h264, stream-fromat=byte-stream, alignment=nal ! h264parse ! avdec_h264  ! videoconvert ! video/x-raw, format=RGB ! appsink max_buffers=1 drop=true sync=false name=as1 "
    )
    ...
    while True:
        ...
        if faceDetection == True:
            ...
            if len(bounding_boxes) > 0:
                ...
                img = img[
                    int(bounding_boxes[0][1]) : int(bounding_boxes[0][3] + 1),
                    int(bounding_boxes[0][0]) : int(bounding_boxes[0][2] + 1),
                    :,
                ]
                ss = img.shape
                if (ss[0] * ss[1]) == 0:
                ...
                face = (
                    cv2.resize(face, (64, 64)).reshape((1, 64, 64, 1)).astype("float32")
                )
                face /= 127.5
                face -= 1.0
